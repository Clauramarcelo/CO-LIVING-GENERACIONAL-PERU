...
function cardTemplate(item){
  ...
  const badgeExtra = item.highlight ? `<span class="badge badge--start">Empieza aquí</span>` : "";
...
function getVisiblePrograms(){
  return data.filter(p => p.tier === "core" || (state.showExtra && p.tier === "extra"));
}
...
function renderFavorites(){
  ...
  // Hook favoritos dentro de favoritos
...
function setupMorePrograms(){
  $("#btnMorePrograms")?.addEventListener("click", () => {
    ...
    renderAll();
    // Llevar suavemente al inicio de programas si se expande
